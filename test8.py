...
sell_order_list = []
sell_list = {}
target_ticker = "BTC-PSG"
print(sell_list)
print(sell_list)
#sell_list[target_ticker] = sell_order_list
#for order in sell_list[target_ticker]:
#    del sell_list[target_ticker]
buy_price = 0.00090000
tmp = upbit.sell_limit_order(f"{target_ticker}", buy_price, 3)
if 'uuid' in tmp:
    sell_order_list.append(tmp['uuid'])
    logger.info("ticker : %s   sell_limit_order : %d", target_ticker, buy_price)
time.sleep(10)
sell_list[target_ticker] = sell_order_list
bought_list = {target_ticker:buy_price}
ref_price = bought_list[target_ticker]
check_price = pyupbit.get_current_price(f"{target_ticker}")
if (ref_price > check_price):
    logger.info("ref_price %0.6f > check_price %0.6f", ref_price, check_price)
