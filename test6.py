...
upbit.buy_market_order(f"{target_ticker}", buy_balance * 0.9)
time.sleep(10)
print("buy_price : %d  buy_balance : %d", buy_price, buy_balance)

sell_balance_tmp = upbit.get_balance(ticker=f"{target_ticker}")
#sell_balance = round(sell_balance_tmp*0.895,2)
sell_balance = sell_balance_tmp * 0.295
sell_price = buy_price * 1.03
tmp = upbit.sell_limit_order(f"{target_ticker}", 4400000, sell_balance)
sell_order_list.append(tmp['uuid'])
print(sell_order_list)
time.sleep(10)
sell_price = buy_price * 1.05
tmp = upbit.sell_limit_order(f"{target_ticker}", 4450000, sell_balance)
sell_order_list.append(tmp['uuid'])
print(sell_order_list)
time.sleep(10)
sell_list[target_ticker] = sell_order_list

for order in sell_list[target_ticker]:
    upbit.cancel_order(order)
    time.sleep(10)
time.sleep(10)
del sell_list[target_ticker]

print(sell_list[target_ticker])
