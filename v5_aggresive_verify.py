...
def check_rate_validity(ticker):
    ...
    for value in close_list:
        if (cnt < 8):
            ...
            abc = 0
        cnt = cnt + 1
    logger.info("avg_value : %d   new_value_1 : %d    new_value_2 : %d", avg_value, new_value_1, new_value_2)
...
def check_volume_validity(ticker):
    ...
    for value in volume_list:
        if (cnt < 8):
            ...
            abc = 0
        cnt = cnt + 1
    logger.info("avg_value : %d   new_value_1/avg_value : %0.3f   new_value_2/avg_value : %0.3f", avg_value, new_value_1/avg_value, new_value_2/avg_value)
    if (new_value_1/avg_value >= 2) and (new_value_2/avg_value >= 2.2):
        return "valid"
    else:
        return "invaild"
