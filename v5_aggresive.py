...
def check_rate_validity(ticker):
    ...
    for old_value, new_value in zip(list_from_old, list_from_new):
        ...
        if (cnt >= 3) and (cnt <= 6):
            ...
            abc = 0
        cnt = cnt + 1
        #logger.info("cnt : %d   value : %f   new_value : %f    old_value : %f", cnt, value, new_value, old_value)
...
def check_volume_validity(ticker):
    avg_volume = 0
    df = pyupbit.get_ohlcv(f"{ticker}", interval="minute1", count=11)
    avg_volume_list = df['volume'][0:8]
    for avg_volume_t in avg_volume_list:
        avg_volume = avg_volume + avg_volume_t
    avg_volume = avg_volume / 8
    new_volume_1 = df['volume'][8] / avg_volume
    new_volume_2 = df['volume'][9] / avg_volume
    #logger.info("avg_volume : %f   new_volume_1 : %f   new_volume_2 : %d", avg_volume, new_volume_1, new_volume_2)
    if (new_volume_1 >= 2) and (new_volume_2 >= 2):
        return "valid"
    else:
        return "invalid"
