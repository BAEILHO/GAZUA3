...
logger.info("START!!   old time : %s", old_time)
tmp = sys.argv[0]
tmp2 = tmp.split('.')
print(tmp)
print(tmp2[0])
...
df = pyupbit.get_ohlcv(f"{ticker}", interval="minute1", count=f"{cnt}")
high_list = df['high'][cnt:cnt_m]
#df_col = df.columns
df_idx = df.index
print(df)
#print(df_col)
...
#high_list = df_t['high'][0:4]
#print(high_list)
#high_list = df_t['high']
#print(high_list)
#low_list = df_t['low'][0:4]
#print(low_list)
#low_list = df_t['low']
#print(low_list)
...
#            abc = 0
#        cnt = cnt + 1
#    if (new_rate_1/avg_rate >= 1.01) and (new_rate_2/avg_rate >= 1.015) and (new_rate_1/avg_rate < 1.1) and (new_rate_2/avg_rate < 1.1):
...
#        return "valid"
#    elif (new_rate_2/avg_rate >= 1.02) and (new_rate_2/avg_rate < 1.1) and (ticker_type == "KRW"):
#        #logger.info("avg_rate : %0.6f   new_rate_1/avg_rate : %0.6f    new_rate_2/avg_rate : %0.6f", avg_rate, new_rate_1/avg_rate, new_rate_2/avg_rate)
#        return "valid"
#    else:
#        return "invalid"
...
#            diff = str(delta_time)
#            diff_int = int(diff[2:4])
#            valid_candle_cnt = sim_cycle - diff_int
#
#            avg_list = (high_list + low_list) / 2
#            avg_price = sum(avg_list)/len(avg_list)
#            sim_profit = avg_price/ref_price - 1
#            sim_total_profit = sim_total_profit + sim_profit
#            sim_total_ticker_cnt = sim_total_ticker_cnt + 1
#            if (sim_profit > 0):
#                sim_plus_cnt = sim_plus_cnt + 1
#            logger.info("sim_profit - %s : %0.6f", ticker, sim_profit)
#    if (sim_total_profit > 1.03):
#        buy_enable = 1
#        logger.info("buy_enable = 1")
#        target_margin = sim_total_profit
#    else:
#        buy_enable = 0
#        logger.info("buy_enable = 0")
#    logger.info("sim_total_profit : %0.6f   sim_plus_cnt/sim_total_ticker_cnt : %d/%d", sim_total_profit, sim_plus_cnt, sim_total_ticker_cnt)
#
#
#
#
